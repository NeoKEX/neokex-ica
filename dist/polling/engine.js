...
export class PollingEngine {
    ...
    async seedSeenIds() {
        try {
            ...
            this.isSeeded = true;
            logger.info(`Seeded ${this.seenMessageIds.size} message IDs across ${threads.length} threads`);
        }
        catch (err) {
    ...
    async startPolling(options = 5_000) {
        ...
        const { interval = 5_000, minInterval = 3_000, maxInterval = 30_000, maxConsecutiveErrors = 5, circuitCooldown = 60_000, } = opts;
        this.isPolling = true;
        ...
        while (this.isPolling) {
            ...
            // ── Poll ──────────────────────────────────────────────────────────────
            try {
                const hadActivity = await withTimeout(this.pollCycle(), POLL_TIMEOUT_MS + 5_000, 'pollCycle');
                this.stats.totalPolls++;
                this.stats.lastPollAt = Date.now();
                this.stats.consecutiveErrors = 0;
                sweepCounter++;
                if (sweepCounter % 20 === 0)
                    this.sweepExpiredReplyHandlers();
                ...
                if (this.stats.consecutiveErrors >= maxConsecutiveErrors) {
                    ...
                    logger.warn(`Circuit breaker opened after ${maxConsecutiveErrors} errors — cooling down ${circuitCooldown / 1000}s`);
